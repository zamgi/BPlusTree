...
namespace System.Collections.Generic
{
    ...
    /// 
    /// </summary>
    internal sealed class SortedBlockList< T > : SortedList_Impl< BPlusTreeBlock< T > >
    {
        ...
        public int BinarySearch( T value )
        {
            ...
            for ( var endIndex = this.Count - 1; i <= endIndex; )
            {
                ...
                var d = this[ middleIndex ].CompareWith4_SortedBlockList( value );
                if ( d == 0 )
                {
                    return (middleIndex);
                }
...
//#if DEBUG
        public override string ToString() => (this.Count == 0) ? "[EMPTY]" : $"block-count: {this.Count}, min: '{this[ 0 ].Min}', max: '{this[ this.Count - 1 ].Max}'";
//#endif
    }
    ...
    /// 
    /// </summary>
    public sealed class BPlusTreeList< T > : IBPlusTree< T >, IReadOnlyCollection< T >
    {
        private const int DEFAULT_BLOCKLIST_CAPACITY = 7;
        private const int DEFAULT_BLOCK_CAPACITY     = 128;
        ...
        public static BPlusTreeList< T > Create( IComparer< T > comparer, int capacity, int blockCapacity = DEFAULT_BLOCK_CAPACITY )
        {
            var blockListCapacity = ((int) (1.0 * capacity / blockCapacity + 0.5) + 25);
            var bptree = new BPlusTreeList< T >( comparer, blockListCapacity, blockCapacity );
            return (bptree);
        }
        ...
        public T GetValue( T t, T defaultValue = default )
        {
            var index = _SortedBlockList.BinarySearch( t );
            if ( index < 0 )
            {
        ...
        public IEnumerable< T > GetValuesBetween( T min, T max )
        {
            ...
            if ( 0 < _Comparer.Compare( min, max ) ) throw (new ArgumentException( "max < min" )); 
            #endregion
        ...
        {
            #region [.check input params.]
            if ( 0 < comparer.Compare( min, max ) ) throw (new ArgumentException( "max < min" )); 
            #endregion
...
//#if DEBUG
        public override string ToString() => (_SortedBlockList.Count == 0) ? "Count: 0" : $"Count: {this.GetCount()}, Min: '{_SortedBlockList.Min.Min}', Max: '{_SortedBlockList.Max.Max}'";
//#endif

        public bool IsReadOnly => false;
        public void Add( T t ) => TryAdd( t );
        public void Clear() => _SortedBlockList.Clear();
