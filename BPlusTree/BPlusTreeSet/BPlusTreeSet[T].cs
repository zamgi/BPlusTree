...
namespace System.Collections.Generic
{
    ...
    internal sealed class SortedBlockSet< T > : RedBlackTree< BPlusTreeBlock< T > >
    {
        ...
        public BPlusTreeBlock< T > FindNearestBlock( T value )
        {
            ...
            for ( ; ; )
            {
                var order = currNode.Item.CompareWith4_SortedBlockSet( value );
                if ( order == 0 )
                {
...
//#if DEBUG
        public override string ToString() => (this.Count == 0) ? "[EMPTY]" : $"block-count: {this.Count}, min: '{this.Min.Min}', max: '{this.Max.Max}'";
//#endif
    }
    ...
    /// 
    /// </summary>
    public sealed class BPlusTreeSet< T > : IBPlusTree< T >, IReadOnlyCollection< T >
    {
        ...
        public bool TryGetValue( T t, out T exists )
        {
            ...
            var suc = block.TryGetValue( t, out exists );
            return (suc);
        }
        ...
        public bool Remove( T t )
        {
            ...
            if ( block.Count == 1 )
            {
                ...
                if ( 0 <= index )
                {
                    var suc = _SortedBlockSet.Remove( block );
                    Debug.Assert( suc );
            ...
            {
                var suc = block.Remove( t );
                return (suc);
            }
        }
        public IEnumerable< T > GetValuesBetween( T min, T max )// => GetValuesBetween( min, max, _Comparer );
        {
...
//#if DEBUG
        public override string ToString() => (_SortedBlockSet.Count == 0) ? "Count: 0" : $"Count: {this.GetCount()}, Min: '{_SortedBlockSet.Min.Min}', Max: '{_SortedBlockSet.Max.Max}'";
//#endif

        public bool IsReadOnly => false;
        public void Add( T t ) => TryAdd( t );
        public void Clear() => _SortedBlockSet.Clear();
